...
{
    public class EditImageByIdCommandHandler : IRequestHandler<EditImageByIdCommand>
    {
        private readonly IUnitOfWork unitOfWork;
        private readonly IImageServiceClient client;

        public EditImageByIdCommandHandler(IUnitOfWork unitOfWork, IImageServiceClient client)
        {
            this.unitOfWork = unitOfWork;
            this.client = client;
        }

        public async Task<Unit> Handle(EditImageByIdCommand request, CancellationToken cancellationToken)
        {
            try
            {
                var validator = new EditImageByIdCommandValidator();
                ...
                var imageRequest = new EditImageByIdRequest
                {
                    Data = request.Data,
                    ContentType = "image/" + request.ContentType.ToString().ToLower()
                };
                var response = client.EditImageByIdAsync(request.Id, imageRequest).Result;
